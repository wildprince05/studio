...
import type { Train, Weather, Maintenance, AlternativeRoute } from '@/lib/types';
import {
  Sheet,
  SheetContent,
  SheetHeader,
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto p-6 pt-0 space-y-6">
              ...
              <CardContent>
                {weather && (
                  <div className="space-y-2">
                    <InfoItem icon={Wind} label="Weather" value={`${weather.condition} at ${weather.location}`} />
                    <InfoItem icon={Thermometer} label="Temperature" value={`${weather.temperature}°C`} />
                  </div>
                )}
                {maintenance && (
                  <div className="mt-4 space-y-2">
                    <InfoItem icon={Wrench} label="Maintenance" value={`${maintenance.description} on ${maintenance.trackSegment}`} />
                  </div>
                )}
                {!weather && !maintenance && (
                    <p className="text-sm text-muted-foreground">No adverse conditions reported.</p>
                )}
              </CardContent>
            </Card>
            ...
            <div>
              ...
              <Button onClick={handleSuggestRoutes} disabled={isLoading} className="w-full">
                {isLoading ? (
            ...
            {results && (
              <Card className="bg-primary/5">
                <CardHeader>
                  <CardTitle className="text-xl flex items-center gap-2"><Route/> Alternative Routes</CardTitle>
                </CardHeader>
                <CardContent>
